...
import org.apache.spark.sql.catalyst.trees.TreePattern.{AGGREGATE_FILTER, DYNAMIC_PRUNING_SUBQUERY, IN_SUBQUERY, SCALAR_SUBQUERY}
import org.apache.spark.sql.execution
import org.apache.spark.sql.execution.{AggregateFilterExec, InSubqueryExec, SparkPlan, SubqueryAdaptiveBroadcastExec, SubqueryExec}
  ...
  def apply(plan: SparkPlan): SparkPlan = {
    plan.transformAllExpressionsWithPruning(
      ...
      case rf: expressions.AggregateFilter =>
        AggregateFilterExec(
          SubqueryExec(s"runtime-filter#${rf.exprId.id}", subqueryMap(rf.exprId.id)),
          rf.exprId)
    }
  }
}
